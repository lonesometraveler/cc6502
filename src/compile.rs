...
impl<'a> CompilerState<'a> {
    ...
    pub fn warning(&self, msg: &str, loc: usize) {
        ...
        match included_in {
            Some(include) => println!(
                "Warning: {} on line {} of {} (included in {} on line {})",
                msg, line, filename, include.0, include.1
            ),
            ...
            match *subscript {
                ...
                _ => {
                    return Err(self.syntax_error(
                        &format!("No subscript for {} index", varname),
                        px.as_span().start(),
                    ))
                }
            }
        ...
        match self.in_scope_variables.last() {
                ...
                match vars.get(varname) {
                    ...
                    None => match self.functions.get(varname) {
                        Some(_var) => match *subscript {
                            ...
                            _ => Err(self
                                .syntax_error("No subscript for functions", px.as_span().start())),
                        },
                        None => Err(self.syntax_error(
                            &format!("Unknown identifier {}", varname),
                            px.as_span().start(),
                        )),
                    },
                }
            ...
            None => {
                match self.variables.get(varname) {
                    Some(_var) => Ok((varname.into(), subscript)),
                    None => match self.functions.get(varname) {
                        Some(_var) => match *subscript {
                            Expr::Nothing => Ok((varname.into(), subscript)),
                            _ => Err(self
        ...
        let res = self
            ...
            .map_primary(|primary| -> Result<Expr, Error> {
                match primary.as_rule() {
                    Rule::int => Ok(Expr::Integer(parse_int(
    ...
    fn compile_statement(&mut self, p: Pair<'a, Rule>) -> Result<StatementLoc<'a>, Error> {
        ...
        match pair.as_rule() {
            ...
            _ => self.compile_statement_ex(pair),
        }
    }
    ...
    fn compile_statement_ex(&mut self, pair: Pair<'a, Rule>) -> Result<StatementLoc<'a>, Error> {
        ...
        match pair.as_rule() {
            ...
            Rule::local_var_decl => self.compile_local_var_decl(pair.into_inner()),
            Rule::for_loop => {
            ...
            Rule::switch_statement => {
                ...
                for i in c {
                    match i.as_rule() {
                        ...
                        Rule::default_case => {
                            cases.push(case_set.clone());
                            let mut default_set =
                                (Vec::<i32>::new(), Vec::<StatementLoc<'a>>::new());
                            let d = i.into_inner();
                            for j in d {
                                match j.as_rule() {
                                    Rule::statement => {
                ...
                Ok(StatementLoc {
                    pos,
                    label: None,
                    statement: Statement::Switch { expr, cases },
                })
            }
            Rule::break_statement => Ok(StatementLoc {
                pos,
                label: None,
                statement: Statement::Break,
            }),
            Rule::continue_statement => Ok(StatementLoc {
                pos,
                label: None,
            ...
            Rule::return_statement => {
                ...
                Ok(StatementLoc {
                    pos,
                    label: None,
                    statement: Statement::Return(return_value),
                })
            }
            Rule::asm_statement => {
                ...
                Ok(StatementLoc {
                    pos,
                    label: None,
                    statement: Statement::Asm(s, size),
                })
            }
            ...
            Rule::goto_statement => {
                ...
                Ok(StatementLoc {
                    pos,
                    label: None,
                    statement: Statement::Goto(s),
                })
            }
            Rule::nothing => Ok(StatementLoc {
                pos,
                label: None,
                statement: Statement::Expression(Expr::Nothing),
            }),
            _ => {
                        ...
                        match p.as_rule() {
                            ...
                            Rule::scattered => {
                                ...
                                if a > 0 && b > 0 {
                                    ...
                                    return Err(self.syntax_error(
                                        "Memory scattering parameters must be positive",
                                        start,
                                    ));
                                }
                                #[cfg(not(feature = "atari7800"))]
                                return Err(self.syntax_error(
                                    "Memory scattering is only available on Atari 7800 ",
                                    start,
                                ));
                            }
                            Rule::holeydma => {
                                ...
                                #[cfg(not(feature = "atari7800"))]
                                return Err(self.syntax_error(
                                    "Holey DMA is only available on Atari 7800 ",
                                    start,
                                ));
                            }
                            Rule::screencode => {
                ...
                Rule::global_id => {
                        ...
                        match p.as_rule() {
                            Rule::pointer => {
                                var_type = match var_type {
                                    ...
                                    _ => {
                                        return Err(self
                                            .syntax_error("Type too complex not supported", start))
                                    }
                                }
                            }
                            Rule::var_const => {
                                    ...
                                    Rule::array_def => {
                                            ...
                                            || var_type == VariableType::ShortPtr
                                        {
                                            ...
                                            for pxx in px.into_inner() {
                                                match pxx.as_rule() {
                                                    Rule::calc_expr => v.push(VariableValue::Int(
                                                    ...
                                                    _ => {
                                                        return Err(self.syntax_error(
                                                            &format!(
                                                            "Expecting constant literal, found {}",
                                                            pxx.as_str()
                                                        ),
                                                            start,
                                                        ))
                    ...
                    if holeydma {
                        if let Some((a, _)) = scattered {
                            if a != 8 && a != 16 {
                                return Err(self.syntax_error(
                                    "Holey DMA is only available for 8 or 16 lines scattering",
                                    start,
                                ));
                            }
                        }
        ...
        match p.as_rule() {
            Rule::local_var_decl_const => {
                ...
                Ok(StatementLoc {
                    pos,
                    label: None,
                    statement: Statement::LocalVarDecl,
                })
            }
                        ...
                        Rule::local_var_type => {
                            for p in pair.into_inner() {
                                match p.as_rule() {
                                    ...
                                    Rule::var_simple_type => {
                                        if p.as_str().starts_with("short")
                                            || p.as_str().starts_with("int")
                                        {
                                            var_type_ex = VariableType::Short;
                                            if !signedness_specified {
                                                signed = true;
                                ...
                                match p.as_rule() {
                                    Rule::pointer => {
                                        var_type = match var_type {
                                            ...
                                            _ => {
                                                return Err(self.syntax_error(
                                                    "Type too complex not supported",
                                                    start,
                                                ))
                                            }
                                        }
                                    ...
                                    Rule::expr_init_value => {
                                        if var_const {
                                            return Err(self.syntax_error(
                                                "Can't dynamically initialize array this way",
                                                start,
                                            ));
                                        }
                                        ...
                                        statements.push(StatementLoc {
                                            pos,
                                            label: None,
                                            statement: Statement::Expression(assign),
                                        });
                                    }
                                    _ => unreachable!(),
                                }
    ...
    fn compile_block(&mut self, p: Pair<'a, Rule>) -> Result<StatementLoc<'a>, Error> {
        ...
        for pair in p.into_inner() {
            match pair.as_rule() {
                Rule::statement => statements.push(self.compile_statement(pair)?),
                _ => {
                    debug!("What's this ? {:?}", pair);
                    unreachable!()
                }
        ...
        for pair in pairs {
            ...
            match pair.as_rule() {
                ...
                Rule::bank => {
                    ...
                    if bank != 0 && inline {
                        return Err(
                            self.syntax_error("Bank spec and inlining are incompatible", start)
                        );
                    }
                }
                Rule::interrupt => {
                    if bank != 0 {
                        return Err(
                            self.syntax_error("Bank spec and interrupt are incompatible", start)
                        );
                    }
                    interrupt = true;
                }
                ...
                Rule::var_simple_type => {
                    if pair.as_str().starts_with("char") {
                        return_type = Some(VariableType::Char);
                    } else {
                        return Err(self.syntax_error(
                            "Only char type is allowed as return type of function",
                            start,
                        ));
                    }
                }
...
pub fn compile<I: BufRead, O: Write>(
    input: I,
    output: &mut O,
    args: &Args,
    builder: fn(&CompilerState, &mut dyn Write, &Args) -> Result<(), Error>,
) -> Result<(), Error> {
    ...
    let pratt = PrattParser::new()
        ...
        .op(Op::infix(Rule::assign, Assoc::Right)
            | Op::infix(Rule::mass, Assoc::Right)
            | Op::infix(Rule::pass, Assoc::Right)
            | Op::infix(Rule::andass, Assoc::Right)
            | Op::infix(Rule::orass, Assoc::Right)
            | Op::infix(Rule::xorass, Assoc::Right)
            | Op::infix(Rule::blsass, Assoc::Right)
            | Op::infix(Rule::brsass, Assoc::Right))
    ...
    let pratt_init_value = PrattParser::new()
        .op(Op::infix(Rule::assign, Assoc::Right)
            | Op::infix(Rule::mass, Assoc::Right)
            | Op::infix(Rule::pass, Assoc::Right)
            | Op::infix(Rule::andass, Assoc::Right)
    ...
    let calculator = PrattParser::new()
        .op(Op::infix(Rule::ternary_cond2, Assoc::Right))
        .op(Op::infix(Rule::ternary_cond1, Assoc::Right))
        .op(Op::infix(Rule::lor, Assoc::Left))
        .op(Op::infix(Rule::land, Assoc::Left))
            ...
            ex.line_col = match e.line_col {
                ...
                LineColLocation::Span((l1, c1), (l2, c2)) => {
                    ...
                    LineColLocation::Span(
                        (mapped_lines[l1 - 1].1 as usize, c1),
                        (mapped_lines[l2 - 1].1 as usize, c2),
                    )
                }
            };
            eprintln!("{}", ex);
