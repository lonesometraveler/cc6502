...
impl<'a, 'b> GeneratorState<'a> {
        ...
        match alternatives {
            Expr::BinOp { lhs, op, rhs } => {
                if *op == Operation::TernaryCond2 {
                    if self.acc_in_use {
                        ...
                        if la != ra {
                            return Err(self.compiler_state.syntax_error(
                                "Different alternative types in ?: expression",
                                pos,
                            ));
                        }
                        ...
                        if let Some(b) = cond {
                            ...
                            let left = self.generate_expr(lhs, pos, false, false)?;
                            let la =
                                self.generate_assign(&ExprType::A(false), &left, pos, false)?;
                            self.asm(JMP, &ExprType::Label(ifend_label.clone()), pos, false)?;
                            self.label(&else_label)?;
                            self.acc_in_use = false;
                            let right = self.generate_expr(rhs, pos, false, false)?;
                            let ra =
                                self.generate_assign(&ExprType::A(false), &right, pos, false)?;
                            self.label(&ifend_label)?;
                    ...
                    Err(self
                        .compiler_state
                        .syntax_error("Missing alternatives in ?: expression", pos))
                }
            }
            _ => Err(self
                .compiler_state
    ...
    pub(crate) fn generate_expr_cond(
        &mut self,
        expr: &Expr,
        pos: usize,
    ) -> Result<ExprType, Error> {
    ...
    fn generate_branch_instruction(
        ...
        match op {
            ...
            _ => Err(Error::Unimplemented {
                feature: "condition statement is partially implemented",
            }),
        }
    }
    ...
    fn generate_branch_instruction_alt(
        &mut self,
        op: &Operation,
        signed: bool,
        label: &str,
                    ...
                    match operator {
                        ...
                        _ => {
                            ...
                            if let ExprType::Immediate(v) = left {
                                if (operator == Operation::Neq && *v != 0)
                                    || (operator == Operation::Eq && *v == 0)
                                {
                            ...
                            if self.carry_flag_ok {
                                ...
                                return self
                                    .generate_branch_instruction_alt(&operator, signed, label);
                            }
                        }
                    }
                }
            ...
            ExprType::Y => {
                ...
                match right {
                    ...
                    ExprType::Absolute(_, eight_bits, _) => {
                        if !eight_bits {
                            return Err(self.compiler_state.syntax_error(
                                "Comparison is only partially implemented on 16 bits data",
                                pos,
                            ));
                        }
                        self.asm(CPY, right, pos, false)?;
            ...
            Expr::BinOp { lhs, op, rhs } => {
                if match op {
                    ...
                    Operation::Land => {
                        if negate {
                            let cond =
                                self.generate_condition(lhs, pos, true, label, immediate_special)?;
                            if let Some(a) = cond {
                                if a {
                                    return Ok(Some(true));
                                }
                            ...
                            let ifstart_label = format!(".ifstart{}", self.local_label_counter_if);
                            self.local_label_counter_if += 1;
                            let cond = self.generate_condition(
                                lhs,
                                pos,
                                true,
                                &ifstart_label,
                    ...
                    Operation::Lor => {
                        if negate {
                            ...
                            let cond =
                                self.generate_condition(lhs, pos, false, label, immediate_special)?;
                            if let Some(a) = cond {
                                if a {
                                    return Ok(Some(true));
                                }
                    ...
                    if immediate_special {
                        if let ExprType::Immediate(a) = l {
                            if let ExprType::Immediate(b) = r {
                                match op {
                                    Operation::Eq => {
                                        return Ok(Some(if a == b { !negate } else { negate }))
                                    }
                                    Operation::Neq => {
                                        return Ok(Some(if a != b { !negate } else { negate }))
                                    }
        ...
        if flags_ok(&self.flags, &expr) {
            ...
            match &expr {
                ...
                ExprType::Absolute(a, eight_bits, b) => {
                    ...
                    if !eight_bits {
                        self.generate_condition_16bits(
                            &expr,
                            if negate {
                                &Operation::Eq
                            } else {
    ...
    pub(crate) fn generate_if(
        ...
        else_body: Option<&'a StatementLoc<'a>>,
        pos: usize,
    ) -> Result<(), Error> {
        self.local_label_counter_if += 1;
        let ifend_label = format!(".ifend{}", self.local_label_counter_if);
        match else_body {
            None => match body.statement {
                Statement::Break => {
                    let brk_label = {
    ...
    pub(crate) fn generate_switch(
        &mut self,
        expr: &'a Expr,
        cases: &'a Vec<(Vec<i32>, Vec<StatementLoc<'a>>)>,
        pos: usize,
    ) -> Result<(), Error> {
        ...
        match self.loops.last() {
            Some(l) => self
                .loops
                .push((l.0.clone(), switchend_label.clone(), false)),
            None => self
                .loops
                .push(("".to_string(), switchend_label.clone(), false)),
        }
        self.local_label_counter_if += 1;
        let mut switchnextstatement_label =
            format!(".switchnextstatement{}", self.local_label_counter_if);
        debug!("Cases : {:?}", cases);
        for (case, is_last_element) in cases
            .iter()
            .enumerate()
            .map(|(i, c)| (c, i == cases.len() - 1))
        {
            self.local_label_counter_if += 1;
            ...
            match case.0.len() {
                ...
                1 => {
                    self.generate_condition_ex(
                        &e,
                        &Operation::Eq,
                        &ExprType::Immediate(case.0[0]),
                        pos,
                        true,
                        &switchnextcase_label,
                    )?;
                ...
                _ => {
                    for i in &case.0 {
                        self.generate_condition_ex(
                            ...
                            pos,
                            false,
                            &switchnextstatement_label,
                        )?;
                    }
                    self.asm(
                        JMP,
                        &ExprType::Label(switchnextcase_label.clone()),
                        0,
                        false,
                    )?;
                    jmp_to_next_case = true;
            ...
            if !is_last_element {
                self.asm(
                    JMP,
                    &ExprType::Label(switchnextstatement_label.clone()),
                    0,
                    false,
                )?;
            }
...
fn flags_ok(flags: &FlagsState, expr_type: &ExprType) -> bool {
    match flags {
        ...
        FlagsState::A => {
            matches!(*expr_type, ExprType::A(_))
        }
        ...
        FlagsState::Absolute(var, eight_bits, offset) => {
            *expr_type == ExprType::Absolute(var.clone(), *eight_bits, *offset)
        }
        _ => false,
    }
}
