...
use crate::assemble::{AsmInstruction, AsmMnemonic, AsmMnemonic::*, AssemblyCode};
use crate::compile::*;
...
impl<'a> GeneratorState<'a> {
    ...
    pub(crate) fn asm(
        ...
        match operand {
            ...
            ExprType::A(s) => {
                ...
                return Err(self
                    .compiler_state
                    .syntax_error("Unexpected expression type", pos));
            }
            ...
            ExprType::AbsoluteY(variable) => {
                ...
                if v.var_type == VariableType::CharPtrPtr || v.var_type == VariableType::ShortPtr {
                    ...
                    match mnemonic {
                        ...
                        STY | LDY | CPY => {
                            return Err(self
                                .compiler_state
                                .syntax_error("Can't use Y addressing on Y operation", pos))
                        }
                        CPX => {
                            return Err(self.compiler_state.syntax_error(
                                "Can't use Y addressing on compare with X operation",
                                pos,
                            ))
                        }
                        STX => {
                            if v.memory != VariableMemory::Zeropage {
                                return Err(self.compiler_state.syntax_error("Can't use Y addressing on a non zeropage variable with X storage", pos));
                            }
                        }
                        _ => (),
                    }
                    ...
                    if v.size == 1 {
                        ...
                        if v.memory != VariableMemory::Zeropage {
                            return Err(self.compiler_state.syntax_error(
                                "Y indirect addressing works only on zeropage variables",
                                pos,
                            ));
                        }
                        nb_bytes = 2;
                        ...
                        indirect = true;
                        match mnemonic {
                            STX | STY | LDX | LDY | CPX | CPY => {
                                return Err(self.compiler_state.syntax_error(
                                    "Can't use Y indirect addressing on X or Y operation",
                                    pos,
                                ))
                            }
                            _ => (),
                        ...
                        return Err(self.compiler_state.syntax_error(
                            "X-Indirect adressing mode not available with Y register",
                            pos,
                        ));
                    }
                } else {
                ...
                if v.memory != VariableMemory::Zeropage || indirect {
                    ...
                    && v.var_type != VariableType::ShortPtr
                {
                    ...
                    match mnemonic {
                        STX | LDX | CPX => {
                            return Err(self
                                .compiler_state
                                .syntax_error("Can't use X addressing on X operation", pos))
                        }
                        CPY => {
                            return Err(self.compiler_state.syntax_error(
                                "Can't use X addressing on compare with Y operation",
                                pos,
                            ))
                        }
                        STY => {
                            if v.memory != VariableMemory::Zeropage {
                                return Err(self.compiler_state.syntax_error("Can't use X addressing on a non zeropage variable with Y storage", pos));
                            }
                        }
                        _ => (),
                    }
        ...
        if let Some(fx) = &self.current_function {
            let code2: AssemblyCode = match self.functions_code.get(f) {
                None => {
                    return Err(self
                        .compiler_state
                        .syntax_error("Inline function must be defined before use", pos))
                }
                Some(c) => c.clone(),
            };
    ...
    fn function_is_actually_in_use(
        &self,
        f: &str,
        functions_actually_in_use: &mut HashSet<String>,
    ) {
