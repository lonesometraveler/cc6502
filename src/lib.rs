...
#[command(
    author,
    about = "A subset of C compiler for the 6502 processor",
    long_about = "A subset of C compiler for the 6502 processor\nCopyright (C) 2023 Bruno STEUX\n\nThis program comes with ABSOLUTELY NO WARRANTY;\nThis is free software, and you are welcome to redistribute it\nunder certain conditions;"
)]
pub struct Args {
    /// Input file name
...
mod tests {
    ...
    fn if_test3() {
        let args = sargs(1);
        let input =
            "unsigned char i, j; void main() { i = 0; j = 0; if (i == 0 && j == 0) X = 1; }";
        let mut output = Vec::new();
        compile(input.as_bytes(), &mut output, &args, simple_build).unwrap();
        let result = str::from_utf8(&output).unwrap();
...
void main() { fn2(); fn3(); }
    ...
    fn params_test4() {
        let args = sargs(1);
        let input =
            "char x, y; void f(char *x, char y) { x[Y = 0] = y; }; void main() { f(&x, y); }";
        let mut output = Vec::new();
        compile(input.as_bytes(), &mut output, &args, simple_build).unwrap();
        let result = str::from_utf8(&output).unwrap();
        print!("{:?}", result);
